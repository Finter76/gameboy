...
void cpu_write_word(CPU* cpu, Bus *bus, u16 address, u16 value){
    bus_write(bus, address, value & 0xFF);
    bus_write(bus, address + 1, (value >> 8) & 0xFF);
}
...
u8 cpu_alu(CPU *cpu, u8 a, u8 b, ALU_OP op) {
    ...
    switch(op) {
        ...
        case ALU_INC:
            old_c = cpu->AF.bytes.lo & FLAG_C;

            result16 = a + 1;
            if((result16 & 0xFF) == 0) f |= FLAG_Z;
            // N = 0;            
            if ((a & 0xF) == 0x0F) f |= FLAG_H;

            cpu->AF.bytes.lo = f | old_c;
            return (u8)result16;
